...
function [MYparent,Mychild,PC,Undirected,ntest,ntime] = ELCS_Z(Data,target,alpha,ns,p,maxk)
    start = tic;
    ntest = 0;
%     all_sepset = cell(1,p);
    DAG = zeros(p,p);
    ...
    Q = target;
    Tmp = [];
    [MB, pc, parent, child, unsure, SpouseY, test, time] = EMB_Z(Data, target, alpha, ns, p, maxk);
    ntest = ntest + test;
    PC = pc;
    MYparent = parent;
    Mychild = child;
    ...
    while length(Tmp)<=p && ~isempty(Q)
        ...
        if PC_calculated(A)
            [MB, pc, parent, child, unsure, SpouseY, test, time] = EMB_Z(Data, A, alpha, ns, p, maxk);
            PC_calculated(A)=0;
            ntest = ntest + test;
        end
