...
function [IDT,idT3,idT3_count,test,already_calculated_MB,pc,time] = CMB_subroutine_Z(data,samples,target,alpha,IDT,maxK,already_calculated_MB,all_MB)
...
[pc,ntest_PC] = HITONPC_Z (data,target,alpha, samples, p, maxK);

test=test+ntest_PC;

Z=[];

[IDT,idT3,idT4,ntest_CS1,idT3_count,idT4_count]=CausalSearch_Z(data,samples,target,alpha,pc,Z,IDT,idT3,idT3_count,idT4,idT4_count);
test=test+ntest_CS1;
...
for i=1:idT4_count
    ...
    if already_calculated_MB(X)
        ...
        [all_MB{X},ntest_MB] = HITONMB_Z (data,X,alpha, samples, p, maxK);
    ...
    [IDT,idT3,idT4,ntest_CS2]=CausalSearch_Z(data,samples,target,alpha,pc,Z,IDT,idT3,idT3_count,idT4,idT4_count);
    test=test+ntest_CS2;
