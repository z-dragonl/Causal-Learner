...
function [MB,test,time] = interIAMBnPC_G2(Data,target,alpha,ns,p,maxK)
%
% interIAMBnPC_G2 finds the Markov blanket of target node on discrete data
%
% INPUT :
    ...
    for i=length(tmp_MB):-1:1
        ...
        while length(SZ) >= cutSetSize&&cutSetSize<=maxK
            ...
            for si=1:length(SS)
                ...
                test=test+1;
                [pval]=my_g2_test(tmp_MB(i),target,S,Data,ns,alpha);        
                if isnan(pval)       
                    CI=0;
                else
                    if pval<=alpha
