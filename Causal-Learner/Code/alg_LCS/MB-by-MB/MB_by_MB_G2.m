...
function [P,C,PC,UN,test,time] = MB_by_MB_G2(Data,target,alpha,ns,p,maxK)
%
% MB_by_MB_G2 finds and distinguishes the parents and children of target node on discrete data
%
% INPUT :
...
while length(Tmp)<=p && ~isempty(Q)
    ...
    % Get MB(A)
    if MB_calculated(A)
        [all_MB{A},ntest1]=IAMB_Forward_G2 (Data,A,alpha,ns,p,maxK);
        test=test+ntest1;
        MB_calculated(A)=0; 
    end
    ...
    for i=1:length(all_MB{A})
        ...
        while length(CanPC) >= cutSetSize &&cutSetSize<=maxK
            ...
            for si=1:length(SS)
                Z = SS{si};
                test=test+1;
                pval=my_g2_test(B,A,Z,Data,ns,alpha);
