...
function [P,C,PC,UN,ntest,time] = PSL_Z(Data,target,alpha,ns,p,k)
...
while length(Tmp)<=p && ~isempty(Q)
    ...
    % Get PC(A)
    if PC_calculated(A)
        [all_PC{A},ntest1,ntime1,all_sepset{A}]=PCsimple_Z (Data,A,alpha,ns,p,k);
...
%     all_PC{A}
    ...
    for i=1:length(all_PC{A})
        ...
        % Get PC(B)
        if PC_calculated(B)
            [all_PC{B},ntest2,ntime2,all_sepset{B}]=PCsimple_Z (Data,B,alpha,ns,p,k);
...
%                 end
                ...
                if ~ismember(A,all_sepset{B}{B_tmp})
                    ntest=ntest+1;
                    pval=my_fisherz_test(B,B_tmp,myunion(A,all_sepset{B}{B_tmp}),Data,samples,alpha);
...
%                         pause;
        ...
        for j=1:length(all_PC{B})
            ...
            if ~ismember(B,all_sepset{A}{C})
                ntest=ntest+1;
                pval=my_fisherz_test(A,C,myunion(B,all_sepset{A}{C}),Data,samples, alpha);
