...
function [P,C,PC,UN,test,time] = PCD_by_PCD_G2(Data,target,alpha,ns,p,maxK)
%
% PCD_by_PCD_G2 finds and distinguishes the parents and children of target node on discrete data
%
% INPUT :
...
while length(Tmp)<=p && ~isempty(Q)
    ...
    % Get PC(A)
    if PC_calculated(A)
        [all_PC{A},ntest1,~,all_sepset{A}]=MMPC_G2 (Data,A,alpha,ns,p,maxK);
        test=test+ntest1;
        PC_calculated(A)=0; 
    end
