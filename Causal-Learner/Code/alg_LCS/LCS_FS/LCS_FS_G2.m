...
function [P,C,PC,UN,test,time] = LCS_FS_G2(Data,target,alpha,ns,p,maxK)
%
% LCS_FS_G2 finds and distinguishes the parents and children of target node on discrete data
%
% INPUT :
...
while length(Tmp)<=p && ~isempty(Q)
    ...
    if PC_calculated(A)
        all_PC{A}=FCBF_PC_G2(Data,A,alpha,ns,p,maxK);
        PC_calculated(A)=0;
    end
    ...
    for i=1:length(all_PC{A})
        ...
        if PC_calculated(B)
            [all_PC{B},ntest_PC]=FCBF_PC_G2(Data,B,alpha,ns,p,maxK);
            PC_calculated(B)=0;
