function [MB, pc, parent, child, unsure, SpouseY, ntest, ntime] = EMB_Z(Data, target, alpha, ns, p, maxk)
...
%       time is the runtime of the algorithm
   [samples,p]=size(Data);
    ...
    [pc, ntesthpc, ~,sepset] = HITONPC_Z(Data,target,alpha, ns, p, maxk);
...
%     mysepsetpc = cell(1,p);
    for i = 1 : length(CanSp)
        ...
        for j = 1 : length(pc)
            ntest = ntest + 1;
            [pvaltmp] = my_fisherz_test(CanSp(i), pc(j), [], Data, samples,alpha);        % use G^2 to test conditional independence
            if isnan(pvaltmp)
                CI = 0;
            else
               if pvaltmp <= alpha
        ...
        ntest = ntest + 1;
        [pvaltmp] = my_fisherz_test(CanSp(i), target, tmp, Data, samples,alpha);        % use G^2 to test conditional independence
        if isnan(pvaltmp)
            CI = 0;
        else
           if pvaltmp <= alpha
        ...
        if CI == 1
            ...
            for k = 1 : length(rest)
                ntest=ntest+1;
                [pval]=my_fisherz_test(target,CanSp(i),[sepset{1,CanSp(i)},rest(k)],Data,samples,alpha);        % use G^2 to test conditional independence
                if isnan(pval)
                    CI=0;
                else
                   if pval<=alpha
    ...
    for i = 1 : length(pc)
        ...
        for j = 1 : length(SpYtmp)
            ntest = ntest + 1;
            [pval,DepSpY(SpYtmp(j))]=my_fisherz_test(SpYtmp(j), pc(i), [], Data, samples,alpha);        % use G^2 to test conditional independence
        end
    %     
            ...
            for j=pc_length:-1:1
                ...
                while length(CanPC) >= cutSetSize &&cutSetSize<=maxk
                    ...
                    for si=1:length(SS)
                        ...
                        ntest=ntest+1;
                        [pval]=my_fisherz_test(X,pc(i),Z,Data,samples,alpha);        % use G^2 to test conditional independence
                        if isnan(pval)
                            CI=0;
                        else
                           if pval<=alpha
        ...
        for j = 1 : length(SpYtmp)
            ...
            while length(CanPC) >= cutSetSize &&cutSetSize<=maxk
                ...
                for si=1:length(SS)
                    Z = SS{si};
                    ntest=ntest+1;
                    [pval]=my_fisherz_test(pc(i), SpYtmp(j),Z,Data,samples,alpha);        % use G^2 to test conditional independence
                    if isnan(pval)
                        CI=0;
                    else
                       if pval<=alpha
        ...
        for j = 1 : length(SpYtmp)
            ...
            while length(pcRest)>=cutSetSize &&cutSetSize<=maxk
                ...
                for si = 1 : length(SS)
                    ConSet = [SS{si}, pc(i)];
                    ntest = ntest + 1;
                    [pval]=my_fisherz_test(target, SpYtmp(j), ConSet, Data, samples,alpha);        % use G^2 to test conditional independence
                    if isnan(pval)
                        CI = 0;
                    else
                       if pval <= alpha
    ...
    for i = 1 : length(pc)
        ...
        while length(pcRest)>=cutSetSize &&cutSetSize<=maxk
            ...
            for si = 1 : length(SS)
                ConSet = [SS{si}, CanSpY];
                ntest = ntest + 1;
                [pval]=my_fisherz_test(target, pc(i), ConSet, Data, samples,alpha);        % use G^2 to test conditional independence
                if isnan(pval)
                    CI = 0;
                else
                   if pval <= alpha
    ...
    for i = 1 : length(unsure)
        for j = i+1 : length(unsure)
            ntest = ntest + 1;
            [pval]=my_fisherz_test(unsure(i), unsure(j), [], Data, samples,alpha);        
            if isnan(pval)
                CI = 0;
            else
               if pval <= alpha
                    ...
                    CI = 1;
                    ntest = ntest + 1;
                    [pval]=my_fisherz_test(unsure(i), unsure(j), target, Data, samples,alpha);       
                    if isnan(pval)
                        CII = 0;
                    else
                       if pval <= alpha
    ...
    if ~isempty(parent)
        for i = 1 : length(rest)
            ntest = ntest + 1;
            [pval]=my_fisherz_test(parent(1), rest(i), [], Data, samples,alpha);        
            if isnan(pval)
                CI = 0;
            else
               if pval <= alpha
            ...
            if CI==0
                ntest = ntest + 1;
                [pval]=my_fisherz_test(parent(1), rest(i), target, Data, samples,alpha);       
                if isnan(pval)
                    CII = 0;
                else
                   if pval <= alpha
