...
function [P,C,PC,UN,test,time] = CMB_G2(Data,target,alpha,ns,p,maxK)
%
% CMB_G2 finds and distinguishes the parents and children of target node on discrete data
%
% INPUT :
...
while length(Tmp)<=p && ~isempty(Q)
    ...
    if already_calculated(A)
        [IDT(A,:),all_idT3{A},all_idT3_count(A),ntest_CMBs1,already_calculated_MB] = CMB_subroutine_G2(Data,ns,A,alpha,IDT(A,:),maxK,already_calculated_MB,all_MB);
    ...
    for i=1:length(IDT3_count)
        ...
        if already_calculated(X)
            [IDT(X,:),all_idT3{X},all_idT3_count(X),ntest_CMBs2,already_calculated_MB] = CMB_subroutine_G2(Data,ns,X,alpha,IDT(X,:),maxK,already_calculated_MB,all_MB);
